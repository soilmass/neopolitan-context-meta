...
| B10 | 2026-05-09 pre-v1.0 audit pass | 14 of 75 skills failed the description-drift gate on first audit (drift 10-28%; threshold <10%). Reproduces the v0.4.0 family-bootstrap dogfood pattern (A60/B4) at scale. Root cause: YAML folded-scalar (`>`) wraps mid-word at hyphens, producing tokens like `Free- standing` (with space) that don't match body's `Free-stand` token. | **Resolved by iteration** — 14 skills' descriptions rewrapped to break only at word boundaries; bodies augmented with descriptive vocabulary so drift words land in body. All 75 skills now pass. **Cross-referenced as A66 in meta-pipeline ledger; shipped in meta-pipeline v0.7.2** as a 6th common drift signal in `skill-author/references/audit-ritual.md`. **Validates the procedure**: drift gate correctly surfaced the issue; iteration to clean was tractable in a single session. |
| B11 | 2026-05-09 v1.0.0-rc1 smoke-read of cross-cutting atoms | The 7 cross-cutting atoms (PR #7) cite `references/*.md` files in their atom-archetype-required References section that **don't exist** in the repo (e.g., `performance-budget-author` cites `references/budget-tables.md`, `references/ci-enforcement-template.md`, `references/three-js-budgets.md` — none present). The 44 family atoms handle this honestly with the v0.7.0 speculative-skill convention ("No external `references/*.md` files yet — first real authoring run will produce a template worth promoting."). The cross-cutting atoms missed this pattern; they were authored as if the references existed. | **Fixed during the rc1 hold** — 7 atoms' References sections rewritten to match the family atoms' honest convention. PATCH-level content fix; no SKILL.md behavior change; no schema change. The rc1 candidacy is unaffected (PATCH content fixes during the hold are explicitly allowed; only MAJOR bumps restart the clock). All 75 skills remain `healthy`. **Validates the smoke-test discipline**: the plan's recommended (not gating) end-to-end smoke read surfaced this gap before v1.0.0 promotion. |
| B12 | 2026-05-10 v1.0.0-rc1 systematic citation-gap scan | Programmatic scan of all 75 SKILL.md files for skill-name citations that don't resolve to a real skill in either library. Found 3 atoms (`runbook-author`, `vision-author`, `weekly-metric-report-author`) cite `comms-plan-author` as a sibling. The skill doesn't exist in either library (no general comms-plan atom is built). `weekly-metric-report-author` already qualifies it as `(deferred sibling)` — the correct pattern; the other two cite it bare. (Separately, `aeo-schema-author` cites `seo-strategy-author` — already correctly framed as "or a future seo-strategy-author if dedicated tool needed".) | **Fixed during the rc1 hold** — `vision-author` and `runbook-author` updated to qualify the `comms-plan-author` citation per the v0.7.0 speculative-skill convention: "(deferred — no atom for general comms-plans yet; user-invocable `draft-comms-plan` covers it if available, otherwise resolve offline)". PATCH-level content fix during the hold; no SKILL.md behavior change; no schema change. The rc1 candidacy is unaffected. **Validates the systematic-scan discipline**: programmatic citation-gap scan surfaced two real cleanup items the v1.0-rc1 audit-skill drift gate didn't catch (drift detects token-level mismatch; this scan detects citation-resolution mismatch). |
